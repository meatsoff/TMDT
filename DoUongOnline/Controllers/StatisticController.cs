...
namespace DoUongOnline.Controllers
{
    public class StatisticController : Controller
    {
        ...
        public ActionResult ThongKeNguoiDung()
        {
            ...
            var user = db.DonHangs.Select(x => x.KhachHang.TenKhachHang).Distinct();
            foreach (var item in user)
            {
                a.Add(db.DonHangs.Count(x => x.KhachHang.TenKhachHang == item));           
            }
            var rep = a;
        ...
        private void Char1(DateTime start, DateTime end)
        {
            ...
            for (int i = 0; i <= num; i++)
            {
                ...
                var q = db.HoaDons.Where(t => t.NgayTaoHD < f2).Sum(t => t.TongTien);
                if (q == null)
                    q = 0;
                tong += (double)q;
                C1sl.Add((Double)q);
        ...
        private void Char3()
        {
            var s = db.ChiTietDonHangs.GroupBy(t => t.SanPham.TenSP).Select(t => new { TenSP = t.Key, total = t.Sum(i => i.SoLuong) }).Take(5);
        ...
        public ActionResult LoiNhuan(String month)
        {
            if (!month.Equals(""))
            {
                ...
                DateTime beginDate1 = DateTime.Parse(month);
                    ...
                    dsdh = db.PhieuNhaps.SqlQuery(query).ToList();
                    double tong = 0;
                ...
                String query1 = "Select * from HoaDon";            
                    ...
                    dshd = db.HoaDons.SqlQuery(query1).ToList();
                    double tong1 = 0;
        ...
        public ActionResult PN(int? page, double min = double.MinValue, double max = double.MaxValue)
        {
            ...
            int pagenum = (page ?? 1);
            {
                var c = db.PhieuNhaps.OrderBy(x => x.IdPN).Take(0);
        ...
        public ActionResult DT(int? page, double min = double.MinValue, double max = double.MaxValue)
        {
            ...
            int pagenum = (page ?? 1);
            {
                var c = db.HoaDons.OrderBy(x => x.IdHD).Take(0);
                return PartialView(c.ToPagedList(pagenum, pagesize));
            }
        }
        [HttpPost]
        public ActionResult DT(String month, int? page, double min = double.MinValue, double max = double.MaxValue)
        {
            ...
            if (!month.Equals(""))
            {
                ...
                dsdh = db.HoaDons.SqlQuery(query).ToList();
                double tong = 0;
